...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            var adminResult = (from row in db.Administrators
                             select row.UId).Max();
            var studentResult = (from row in db.Students
                              select row.UId).Max();
            var profResult = (from row in db.Professors
                              select row.UId).Max();

            Console.WriteLine(profResult);
            List<string> stringList = new List<string>() {adminResult, studentResult, profResult};
            string lexicalMax = (from str in stringList
                                 orderby str descending
                                 select str).First();
            int maxValue = Int32.Parse(lexicalMax.Substring(1));
            String uID = "u" + (maxValue + 1).ToString();
            try
            {
                ...
                else if(role == "Student")
                {
                    ...
                    student.Subject = departmentAbbrev;
                    student.UId = uID;
