    ...
    public class CommonController : Controller
    {
        ...
        public IActionResult GetDepartments()
        {
            var query = from i in db.Departments
                        select new { name = i.Name, subject = i.Subject };
        ...
        public IActionResult GetCatalog()
        {
            var query = from i in db.Departments
                        where i.Courses.Any()
                        select new { subject = i.Subject, dname = i.Name,
                        courses = from j in i.Courses
                                  select new {number = j.Number, cname = j.Name }
                        };
        ...
        {
            var query1 = from i in db.Courses
                         where i.Subject == subject && i.Number == number
                         select i.CId;

            var query = from i in db.Classes
                        where i.Course == query1.ToArray()[0] 
                        join p in db.Professors
                        on i.Professor equals p.SId
                        into fullTable
                        from f in fullTable
